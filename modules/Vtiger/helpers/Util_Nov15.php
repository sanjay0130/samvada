...
class Vtiger_Util_Helper {
	...
	public static function formatDateDiffInStrings($dateTime) {
		...
		if ($seconds < 60)	return $prefix . self::pluralize($seconds,	"LBL_SECOND") . $suffix. $actual_date;
		if ($minutes < 60)	return $prefix . self::pluralize($minutes,	"LBL_MINUTE") . $suffix. $actual_date;
		if ($hours < 24)	return $prefix . self::pluralize($hours,	"LBL_HOUR") . $suffix. $actual_date;
		if ($days < 30)		return $prefix . self::pluralize($days,		"LBL_DAY") . $suffix. $actual_date;
		if ($months < 12)	return $prefix . self::pluralize($months,	"LBL_MONTH") . $suffix. $actual_date;
		if ($months > 11)	return $prefix . self::pluralize(floor($days/365), "LBL_YEAR") . $suffix. $actual_date;
