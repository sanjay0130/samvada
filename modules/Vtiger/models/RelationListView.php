...
class Vtiger_RelationListView_Model extends Vtiger_Base_Model {
	...
	public function getEntries($pagingModel) {
		...
		if($orderBy) {
			...
			// if documents or calendar or invoice then sort asc instead of desc
			if($relationModuleName == 'Documents' || $relationModuleName == 'Invoice') {
				$query .= ' ORDER BY vtiger_crmentity.createdtime ASC';
			} else if($relationModuleName == 'Calendar' || $relationModuleName == 'Events') {
				$query .= ' ORDER BY vtiger_activity.date_start ASC, vtiger_activity.time_start ASC';
			} else {
