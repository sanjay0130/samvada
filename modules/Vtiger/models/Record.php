...
class Vtiger_Record_Model extends Vtiger_Base_Model {
	 ...
	 */
	public static function getSearchResultnew($searchKey, $module=false) {
...
/*
		echo 'Module-------' . print_r($module); echo '<br>';
		echo 'Searchkey-------' . $searchKey; echo '<br>';
		die;
		*/
		...
		if ($module !== false && is_array($module)) {
			// Search across specific modules only
			$modulesList = implode("','", array_map('addslashes', $module));
			$query .= " AND setype IN ('$modulesList')";
		} elseif (is_string($module)) {
			// Backward compatibility if passed as string
			$query .= ' AND setype = ?';
			$params[] = $module;
		}
		...
		*/
		...
		if ($module == false || in_array('Potentials', $module)) {
		...
		//echo 'Query --------------' . $db->printRawQuery($query, $params); die;
