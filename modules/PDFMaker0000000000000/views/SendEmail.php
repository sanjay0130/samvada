...
class PDFMaker_SendEmail_View extends Vtiger_ComposeEmail_View {
    ...
    public function populateTo($request, $moduleName){
        ...
        if($moduleName == 'Leads')
        {
            $emailValue = $recordModel->get('email');
            $fieldValue = $inventoryRecordId;      
        ...
        }else if($moduleName == 'TaxReceipts')
        {
            /*$related_to = $recordModel->get('related_to');
            $leadsRecordModel = Vtiger_Record_Model::getInstanceById($related_to, 'Leads');
            $emailValue = $leadsRecordModel->get('email');
            $fieldValue = $related_to;*/
            $emailValue = $recordModel->get('email');
            $fieldValue = $inventoryRecordId;
            $related_to = $recordModel->get('related_to');
            $fieldLabel = decode_html(Vtiger_Util_Helper::getRecordName($related_to));
            if(!empty($emailValue)){
                $to[] = $emailValue;
                $to_info[$fieldValue][] = $emailValue;
                $toMailNamesList[$fieldValue][] = array('label' => decode_html($fieldLabel), 'value' => $emailValue);
