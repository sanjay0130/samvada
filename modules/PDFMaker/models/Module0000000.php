...
class PDFMaker_Module_Model extends Vtiger_Module_Model {
    ...
    private $profilesActions = array("EDIT" => "EditView", // Create/Edit
    ...
    */
    ...
    private function setLicenseInfo() {
        ...
        $sql = "SELECT version_type, license_key, license_due_date FROM vtiger_pdfmaker_license";
    ...
    public function GetAvailableTemplates($currModule, $forListView = false, $recordId = false) {
        ...
        $sql = "SELECT vtiger_pdfmaker_displayed.*, vtiger_pdfmaker.templateid, vtiger_pdfmaker.filename, vtiger_pdfmaker.description                
                ...
                LEFT JOIN vtiger_pdfmaker_displayed USING(templateid)
                WHERE vtiger_pdfmaker.module=? AND vtiger_pdfmaker.deleted = ?" . $where_lv . "                                    
...
//      when only one template is available for module, then set it as default
    ...
    public function GetDefaultTemplates($default_mode,$relmodule) {
        ...
        if ($default_mode == "2") $forListView = true;
    ...
    public function GetLicenseDueDate($formated = false) {
        ...
        if ($formated) {
            if ($this->license_due_date != "" && $this->license_due_date != "0000-00-00") {
    ...
    public function SetLicenseDueDate($license_due_date) {
        ...
        $adb = PearDatabase::getInstance();
        $adb->pquery("UPDATE vtiger_pdfmaker_license SET license_due_date = ?",array($license_due_date));
    }
    ...
    public function addAwesomeStyle(& $content, $convert = true){
        ...
        if ($convert) $fontawesomeclass = str_replace("FontAwesome","fontawesome",$fontawesomeclass);

        $style_content = '<style>'.$fontawesomeclass.'</style>';
        if ($convert) $style_content .= '<style>.fa { font-family: fontawesome;}</style>';
        if (empty($content)){
