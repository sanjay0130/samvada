...
class PDFMaker_IndexAjax_Action extends Vtiger_Action_Controller {
    ...
    public function getPreviewContent(Vtiger_Request $request){
        ...
        }				$is_balance=0;		if ($request->has("is_balance") && !$request->isEmpty("is_balance")) {
            $is_balance = $request->get("is_balance");    
        }
        $GeneratePDF->set("generate_type",$generate_type);        $GeneratePDF->set("is_balance",$is_balance);
