...
class SQLReports extends CRMEntity {
	...
	function save_module($module) {
            global $adb;
            $sanitizedSQL = $this->sanitizeSQLCode($_REQUEST['reportsql']);
            $sql = "update vtiger_sqlreports set reportsql = ? where sqlreportsid = ?";
            $res = $adb->pquery($sql, array($sanitizedSQL, $this->id));
	}
	...
	function vtlib_handler($moduleName, $event_type) {
        ...
        function sanitizeSQLCode($sqlCode)
        {
            $found = 0;
            ...
            $sqlCodeLower = strtolower($sqlCode);
            foreach($forbiddenArray as $token)
            {
               $token = strtolower($token);
               if(strstr($sqlCodeLower,$token))
                    $found = 1;
            }
            
            if($found)
            {
                $sqlCodeLower = str_replace($forbiddenArray, "--FORBIDDEN KEYWORD--", $sqlCodeLower);
                return $sqlCodeLower;
            }
