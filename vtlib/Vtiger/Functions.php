...
class Vtiger_Functions
{
	...
	static function validateImage($file_details)
	{
		...
		$saveimage = 'true';
		$log->debug("Validating image file type: $filetype");
		if (!in_array($filetype, $allowedImageFormats)) {
			$log->debug('file type not matched allowed formats');
			$saveimage = 'false';
		}
		$log->debug("File MIME Type in validateImage => $mimeType" . "  " . $saveimage);
		...
		if ($saveimage == 'true') {
			...
			if ($imageContents !== false) {
				// Only treat a real PHP open tag as suspicious
				if (stripos($imageContents, '<?php') !== false || stripos($imageContents, '<?=') !== false) {
					$log->debug('Php injection suspected: real PHP tag found');
					$saveimage = 'false';
				} else {
					$log->debug('Php injection check passed (no real PHP tags found)');
				}
			} else {
				$log->debug('Could not read image contents for PHP injection check (IGNORED)');
			}
		}
		$log->debug("File MIME Type in validateImage => $mimeType" . "  " . $saveimage);
		return $saveimage;
