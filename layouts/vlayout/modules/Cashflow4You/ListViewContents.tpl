	...
	<table class="table table-bordered listViewEntriesTable">
		...
		<tr class="listViewEntries" data-id='{$LISTVIEW_ENTRY->getId()}' data-recordUrl='{$LISTVIEW_ENTRY->getDetailViewUrl()}' id="{$MODULE}_listView_row_{$smarty.foreach.listview.index+1}">
			...
			{assign var=LISTVIEW_HEADERNAME value=$LISTVIEW_HEADER->get('name')}
                        {assign var=LISTVIEW_FIELD_TYPE value=$LISTVIEW_HEADER->getFieldDataType()}
			<td class="listViewEntryValue {$WIDTHTYPE}" data-field-type="{$LISTVIEW_HEADER->getFieldDataType()}" nowrap {if $LISTVIEW_FIELD_TYPE eq "currency" || $LISTVIEW_FIELD_TYPE eq "integer" || $LISTVIEW_FIELD_TYPE eq "double"}style="text-align:right"{/if}>
				...
				{else}
                    {if $LISTVIEW_HEADER->getFieldDataType() eq 'double'}
                       <span style='color:{$LISTVIEW_ENTRY->getCollor($LISTVIEW_HEADERNAME)};'>{decimalFormat($LISTVIEW_ENTRY->get($LISTVIEW_HEADERNAME))}</span>
                    {else}
                        {$LISTVIEW_ENTRY->get($LISTVIEW_HEADERNAME)}
                    {/if}
				{/if}
