                                ...
                                <select class="select2 span5" name="select_module" data-validation-engine='validate[required]]'>
                                    {foreach item=MODULE_MODEL from=$RELATED_MODULES}
                                        {if $MODULE_MODEL->isActive()}
                                            <option value="{$MODULE_MODEL->getRelationModuleName()}" {if $MODULE_MODEL->getRelationModuleName() eq $BLOCK_DATA['module']}selected{/if}>
                                                {vtranslate($MODULE_MODEL->get('label'), $MODULE_MODEL->getRelationModuleName())}</option>
                                        {/if}
                                    {/foreach}
                                </select>
                            </div>
                        ...
                        <div class="control-group">
                            ...
                            <div class="controls row-fluid">
                                <select class="select2 span5" name="after_block">
                                    {foreach from=$ALL_BLOCK_LABELS item=BLOCK_LBL}
                                        <option value="{$BLOCK_LBL}" {if $BLOCK_LBL eq $BLOCK_DATA['after_block']}selected{/if}>{vtranslate($BLOCK_LBL,$SOURCE_MODULE_NAME)}</option>
                                    {/foreach}
                                </select>
                            </div>
                        </div>
