			...
			<div class="pull-right" style="margin-top: -5px;">
				{if $CALENDAR_MODEL->isPermitted('CreateView')}
					<button class="btn addButton btn-sm btn-default createActivity toDotask textOverflowEllipsis max-width-100"
						title="{vtranslate('LBL_ADD_TASK',$MODULE_NAME)}" type="button" href="javascript:void(0)"
					...
					<button class="btn addButton btn-sm btn-default createActivity textOverflowEllipsis max-width-100"
						title="{vtranslate('LBL_ADD_EVENT',$MODULE_NAME)}" data-name="Events"
								...
								<div class='col-lg-7 col-md-7 col-sm-7 textAlignLeft' style="margin-left: -38px;">
									<div class="summaryViewEntries" style="width: 120% !important;">
										...
										{if $EDITVIEW_PERMITTED == 'yes'}<a
												...
												title="{vtranslate('LBL_EDIT',$MODULE_NAME)}"></i></a>{/if}&nbsp;
										{if $DELETE_PERMITTED == 'yes'}<a onclick="Vtiger_Detail_Js.deleteRelatedActivity(event);"
												data-id="{$RECORD->getId()}" data-recurring-enabled="{$RECORD->isRecurringEnabled()}"
												class="fieldValue"><i class="summaryViewEdit fa fa-trash "
												title="{vtranslate('LBL_DELETE',$MODULE_NAME)}"></i></a>{/if}

										{if $RECORD->get('description') neq ''}
											<div class="activityDescription"
												style="margin-top:5px; color:#555; overflow:visible; white-space:normal;">
												{$RECORD->get('description')|nl2br}
											</div>
										{/if}
									...
									<div class="row">
										{if $RECORD->get('activitytype') eq 'Task'}
											...
											<div class="pull-right">
												...
												<strong><span
														class="value picklist-color picklist-{$FIELD_MODEL->getId()}-{Vtiger_Util_Helper::convertSpaceToHyphen($RECORD->get('status'))}">{vtranslate($RECORD->get('status'),$MODULE_NAME)}</span></strong>&nbsp&nbsp;
												{if $EDITVIEW_PERMITTED == 'yes'}
													<span class="editStatus cursorPointer"><i class="fa fa-pencil"
															title="{vtranslate('LBL_EDIT',$MODULE_NAME)}"></i></span>
										...
										{else}
											...
											<div class="pull-right">
												...
												<strong><span
														class="value picklist-color picklist-{$FIELD_MODEL->getId()}-{Vtiger_Util_Helper::convertSpaceToHyphen($RECORD->get('eventstatus'))}">{vtranslate($RECORD->get('eventstatus'),$MODULE_NAME)}</span></strong>&nbsp&nbsp;
												{if $EDITVIEW_PERMITTED == 'yes'}
													<span class="editStatus cursorPointer"><i class="fa fa-pencil"
															title="{vtranslate('LBL_EDIT',$MODULE_NAME)}"></i></span>
													<span class="edit hide">
