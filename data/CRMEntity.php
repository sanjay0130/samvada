...
class CRMEntity
{
	...
	function uploadAndSaveFile($id, $module, $file_details, $attachmentType = 'Attachment')
	{
		...
		if ($attachmentType == 'Image' || ($file_details['size'] && $mimeTypeContents[0] == 'image')) {
			...
			$log->debug("File MIME Type in uploadAndSaveFile => $mimeType". "  ".$save_file);
		}
		...
		if(!$upload_status) {
			$log->debug("File Copy failed from $filetmp_name to " . $upload_file_path . $current_id . "_" . $encryptFileName);
			// log error_get_last as string
			$log->debug(print_r(error_get_last(), true));
		}
